...
class _AddEditTransactionPageState extends State<AddEditTransactionPage> {
  ...
  void _onSave(BuildContext context, List<Category> availableCategories) {
    ...
    if (amount == null || amount <= 0) {
      setState(() => _errorMessage = 'Please enter a valid amount greater than zero.');
      return;
    }
    ...
    final category = _selectedCategory ??
        (availableCategories.isNotEmpty
            ...
            : Category(
                ...
                icon: 'more_horiz',
                color: 0xFF64748B,
                type: _selectedType == TransactionType.expense ? CategoryType.expense : CategoryType.income,
    ...
    return BlocBuilder<CategoryCubit, CategoryState>(
      builder: (context, catState) {
        ...
        if (catState is CategoryLoaded) {
            categories = isIncome ? catState.incomeCategories : catState.expenseCategories;
            // Select default or matching category if not set
            if (_selectedCategory == null) {
              if (widget.existingTransaction != null) {
                final match = catState.allCategories.where((c) => c.id == widget.existingTransaction!.categoryId);
                  ...
                  children: [
                          ...
                          children: [
                            if (_selectedCategory != null) ...[
                              Container(
                                ...
                                height: 36,
                                decoration: BoxDecoration(
                                  color: Color(_selectedCategory!.color).withAlpha(30),
                                  shape: BoxShape.circle,
                                ),
                                child: Icon(
                                  CategoryIconHelper.getIconData(_selectedCategory!.icon),
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: isIncome ? 'e.g. Freelance payment, gift' : 'e.g. Lunch with team, grocery run',
